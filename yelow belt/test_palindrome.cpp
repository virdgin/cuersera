...
void test()
{
  ...
  Assert(IsPalindrom("qeeq"), "");
  Assert(IsPalindrom("qwq"), "");

  Assert(IsPalindrom("qqqq"), "");
  Assert(IsPalindrom("q"), "");
  Assert(!IsPalindrom("qweesq"), "");

  Assert(IsPalindrom("madam"), "");
  Assert(IsPalindrom("wasitacaroracatisaw"), "");
  Assert(!IsPalindrom(" maam"), "");
  Assert(IsPalindrom(" maam "), "");
  Assert(!IsPalindrom("qwe."), "");
  Assert(IsPalindrom("tttt"), "");

  Assert(!IsPalindrom("meveevem!"), "");
  Assert(!IsPalindrom("pppr"), "");

  Assert(!IsPalindrom("qweewqww"), "");
  Assert(!IsPalindrom("jssj1"), "");
  Assert(!IsPalindrom("pwrjuuj"), "");
  Assert(!IsPalindrom("wa s ita car o ra cat isa w"), "");
  Assert(!IsPalindrom("qscvbvxknvbvcsq"), "");
  Assert(!IsPalindrom("jjjrtjjj"), "");
}
